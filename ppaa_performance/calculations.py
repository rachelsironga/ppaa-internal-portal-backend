...
from .models import Objective, Target, Activity, QuarterlyData, KPIActual
...
def target_operational_score(target, financial_year=None):
    ...
    for act in activities:
        ...
        if financial_year:
            qs = qs.filter(financial_year=financial_year)
        # Use computed_ai_percent if available (average or sum by quarter then weight)
        recs = list(qs)
        if not recs:
            continue
        # Average AI% across quarters for this activity, then multiply by weight
