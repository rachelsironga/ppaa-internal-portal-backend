...
class ComputerView(APIView):
    ...
    def get(self, request, uid=None):
        try:
            if uid:
                ...
                return CustomResponse.success(data=ComputerSerializer(computer).data)
            ...
            if computers.exists():
                return CustomPagination.paginate(view_class=self, results=computers, request=request)

            return CustomResponse.errors(message="Computers not found", data=[])
        except Exception as e:
            return CustomResponse.server_error(message=f'Failed to Retrieve Computers: {str(e)}')
    ...
    def post(self, request):
        try:
            with transaction.atomic():
                ...
                if uid:
                    try:
                        instance = Computer.objects.get(uid=uid, is_deleted=False)
            ...
            return CustomResponse.server_error(message=f'Failed to Change Computer: {str(e)}')
    ...
    def delete(self, request, uid):
        try:
            ...
            return CustomResponse.server_error(message="Something went wrong While Deleting Computer")
